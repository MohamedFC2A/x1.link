...
export async function getVpsTelemetry(config: Partial<VpsConfig> = {}): Promise<any> {
  const mergedConfig: VpsConfig = { ...DEFAULT_VPS_CONFIG, ...config };
...
export default async function handler(req: any, res: any) {
  ...
  try {
    ...
    if (req.method === 'GET' || action === 'telemetry') {
    ...
    if (req.method === 'POST') {
      ...
      const automationAction = body?.automationAction || body?.action;

      if (automationAction && ['stop', 'pause', 'start', 'restart', 'status'].includes(automationAction)) {
        const result = await controlAutomation(automationAction);
    ...
    console.error('[API VPS Error]:', err);
    return res.status(500).json({
      error: err.message || 'Internal VPS service error',
      statusNotice: VPS_STATUS_NOTICE,
    });
  }
}
