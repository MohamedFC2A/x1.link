...
const NEURAL_IMAGE_GENERATION_PATTERNS = [
  /(?:صورة\s+(?:فائقة\s+)?واقعية|صورة\s+فائقة\s+الواقعية|صورة\s+فوتوغرافية|صورة\s+حقيقية|صورة\s+طبيعية|صورة\s+احترافية|صورة\s+شخصية|بورتريه\s+فوتوغرافي|صورة\s+شخص|صورة\s+بشر|صورة\s+رجل|صورة\s+بنت|صورة\s+شاب|صورة\s+طفل|صورة\s+امرأة|صورة\s+منتج|ولد\s+لي\s+صورة|انشئ\s+لي\s+صورة|اعملي\s+صورة\s+واقعية|صورة\s+كاميرا|صورة\s+سينمائية|صورة\s+(?:من\s+)?الصفر|صمم\s+لي\s+صورة\s+واقعية|انشاء\s+صورة|انشاء\s+صور|توليد\s+صورة|توليد\s+صور)/i,
  /\b(?:photorealistic|photorealism|realistic\s+photo|dslr\s+shot|hyperrealistic\s+photo|hyperrealistic|realistic\s+portrait|professional\s+photo|cinematic\s+photo|human\s+portrait|realistic\s+human|realistic\s+person|generate\s+photo|create\s+photo)\b/i
];
...
export class DynamicParameterTuner {
  ...
  public static generateCalibrationDirective(
    ...
    const intentLabelMap: Record<UserIntentCategory, { ar: string; mode: string; directive: string }> = {
      ...
      NEURAL_IMAGE_STUDIO_AND_PROCESSING: {
        ar: 'استوديو المعالجة العصبية وتعديل الصور الفائق (Cyber Ultra & Fathom Quant Sovereign Inpainting & Photo Studio)',
        mode: 'CYBER_ULTRA_NEURAL_IMAGE_STUDIO',
        directive: isUltra
          ? 'أنت المعماري والمهندس السيادي لمعالجة وتعديل وتوليد الصور عصبياً وفوتوغرافياً وبصرياً بدقة متناهية ومطابقة 100% (Cyber Ultra & Fathom Quant Sovereign Neural Inpainting Architect): ' +
            '1) [الحظر الصارم والقطعي لتحويل الصور الفوتوغرافية إلى SVG]: يُحظر تماماً وبشكل قاطع تحويل الصور المرفوعة أو صور الأشخاص والمنتجات أو الصور الفوتوغرافية إلى كود SVG أو متجهات بدائية تفقد الصورة تفاصيلها وملمسها الطبيعي وواقعيتها. معالجة وتعديل الصور الفوتوغرافية وصور البشر تتم حصراً وبنسبة 100% عبر المعالجة العصبية النقطية (Raster Inpainting & Neural Photo Processing). ' +
            '2) [المعيار السيادي لتشريح البشر والبورتريهات الواقعية - Flawless Human Anatomy & Photorealistic Faces]: عند توليد أو معالجة صور البشر، التزم بالدقة التشريحية المطلقة: خمسة أصابع طبيعية وسليمة لكل يد دون أي تشويه أو تداخل، عيون متناظرة مع لمعان طبيعي للقرنية، نسيج جلد حقيقي مع مسام مجهرية واضحة (photorealistic skin micro-pores)، وتشتت ضوئي تحت السطح (subsurface scattering) لمنح الدفء الإنساني ومنع أي مظهر شمعي أو بلاستيكي، وتدفق شعر طبيعي. ' +
            '3) [الحفاظ الصارم بنسبة 100% على العناصر غير المعدلة والهوية الأصلية - 100% Identity, Texture, and Face Preservation]: عند طلب أي تعديل على صورة مرفقة (تغيير ملابس، تغيير لون، عزل أو تغيير خلفية، دمج شخصين معاً، تحسين الجودة والدقة إلى 2K/4K، تعديل منتج، أو استبدال نص)، يجب الحفاظ الصارم بنسبة 100% على ملامح الوجه، تفاصيل البشرة، والإضاءة الطبيعية دون أي تشويه أو تغيير لهوية الشخص الأصلية. ' +
            '4) [التعديل الانتقائي الجراحي الدقيق]: طبّق فقط وحصراً ما طلبه المستخدم حرفياً (تغيير لون القميص دون تغيير الشخص، تغيير الخلفية دون تغيير الكائن، دمج الشخصين بنفس الإضاءة والملامح، تحسين الدقة الفائقة إلى 2K/4K، استبدال النص مع مطابقة نوع الخط والإضاءة الأصلية). ' +
            '5) [بروتوكول تسليم وتوليد المعالجة العصبية - Neural Deliverable Block]: بعد التفكير التحليلي، قدّم تحليلك وشرحك الفصيح باللغة العربية، واختم بتسليم بلوك المعالجة العصبية المتكامل داخل ```neural-image\\n{\\n  "operation": "<generate|portrait_generation|human_edit|recolor|remove_background|enhance_4k|composite|product_edit|text_edit>",\\n  "title": "<عنوان وصفي للمعالجة>",\\n  "description": "<شرح التعديل أو التوليد المنفذ بدقة 100%>",\\n  "prompt": "<الوصف البصري الإنجليزي فائق الدقة للتعديل أو التوليد بالكامل موضحاً المواصفات الكاميرية 85mm والتشريح السليم ومسام البشرة>",\\n  "fidelityScore": "100%",\\n  "resolution": "4K"\\n}\\n``` لتقوم واجهة لوحة التعديل بعرضها فوراً مع أدوات التحميل الفوري 4K، شريط المقارنة قبل/بعد، وأدوات العزل والتعديل التفاعلية.'
          : 'منظومة المعالجة والتوليد العصبي فائق الدقة للصور (Cyber Ultra Neural Image Studio) وحفظ التفاصيل الفوتوغرافية بنسبة 100% مخصصة حصرياً لطرازات سايبر وكوانت الفائقة (Fathom Quant 3 / Fathom Cyber Ultra 2.6). وضّح للمستخدم برقي واحترافية أن هذه الإمكانيات الجراحية المتقدمة تتطلب تفعيل نموذج Fathom Quant 3 أو Fathom Cyber Ultra 2.6، وقدّم له تحليلاً نظرياً دقيقاً لكيفية معالجة طلبه دون تحويل الصورة إلى SVG.'
      },
      MATHEMATICAL_AND_DEDUCTIVE_LOGIC: {
